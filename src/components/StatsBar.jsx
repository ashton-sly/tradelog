...
export default function StatsBar({ entries }) {
  ...
  const losses = entries.filter(e => !WIN_GRADES.has(e.grade))
  const winRate = entries.length ? ((wins.length / entries.length) * 100).toFixed(1) : null
  ...
  const bestDay = entries.length ? Math.max(...entries.map(e => e.pnl)) : null
  const worstDay = entries.length ? Math.min(...entries.map(e => e.pnl)) : null

  const stats = [
    { label: 'Total PNL', value: fmt(totalPnl), color: pnlColor(totalPnl) },
    { label: 'Win Rate', value: winRate !== null ? `${winRate}%` : '—', color: '#f1f5f9' },
